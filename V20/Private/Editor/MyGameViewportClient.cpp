...
class FMyGameViewDrawer : public FViewElementDrawer
{
public:
	FMyGameViewDrawer(FMyWidget* MyWidget) : MyWidget(MyWidget) {};

	void Draw(const FSceneView* View, FPrimitiveDrawInterface* PDI)
	{
		if (MyWidget && MyWidget->bDrawWidget)
		{
			MyWidget->Render(View, PDI);
		}
	}
...
void UMyGameViewportClient::Init(struct FWorldContext& WorldContext, UGameInstance* OwningGameInstance, bool bCreateNewAudioDevice)
{
	...
	WidgetMode = WM_None;
	MyWidget = new FMyWidget(this);

	MyGameInstance = Cast<UMyGameInstance>(OwningGameInstance);
	if (!MyGameInstance)
	{
...
void UMyGameViewportClient::BeginDestroy()
{
	delete MyWidget;
...
EMouseCursor::Type UMyGameViewportClient::GetCursor(FViewport* Viewport, int32 X, int32 Y)
{
	//Viewport->InvalidateHitProxy();
	//Viewport->Draw();
	if (bEditMode && !bDragging)
	{
		HHitProxy* HitProxy = Viewport->GetHitProxy(X, Y);
		if (HitProxy)
		{
			CheckHoveredHitProxy(HitProxy);
			if (HitProxy->IsA(HMyWidgetAxis::StaticGetType()))
			{
				return HitProxy->GetMouseCursor();
			}
		}
		else
		{
			bHovered = false;
			MyWidget->CurrentAxis = EAxisList::None;
		}
	}
	if (bUseIcon)
	{
		Super::GetCursor(Viewport, X, Y);
...
void UMyGameViewportClient::CheckHoveredHitProxy(HHitProxy* HoveredHitProxy)
{
	...
	if (HoveredHitProxy)
	{
		if (HoveredHitProxy->IsA(HMyWidgetAxis::StaticGetType()) && !bMouseButtonDown)
		{
			...
			EAxisList::Type ProxyAxis = ((HMyWidgetAxis*)HoveredHitProxy)->Axis;
			NewAxis = ProxyAxis;
			bHovered = true;
		}
		else
		...
		if (NewAxis != MyWidget->CurrentAxis)
		{
			MyWidget->CurrentAxis = NewAxis;
...
bool UMyGameViewportClient::InputKey(FViewport* Viewport, int32 ControllerId, FKey Key, EInputEvent EventType, float AmountDepressed, bool bGamepad)
{
	if (bEditMode)
	{
		MyWidget->bAbsoluteTranslationInitialOffsetCached = false;
		MyWidget->ResetDeltaRotation();
		if (Key == EKeys::LeftMouseButton && EventType == EInputEvent::IE_Pressed)
		{
			if (bHovered)
			{
				bDragging = true;
				TActorIterator<APlayerController> Itr(GetWorld());
				Itr->GetPawn()->DisableInput(*Itr);
			}
			else
			{
				TActorIterator<APlayerController> Itr(GetWorld());
				...
				MyObjectTypes.Add(UEngineTypes::ConvertToObjectType(ECollisionChannel::ECC_Destructible));
				if (Itr->GetHitResultUnderCursorForObjects(MyObjectTypes, true, HitResult))
				{
					SetSelectedActor(HitResult.GetActor());
				}
			}
		}
		else if(Key == EKeys::LeftMouseButton && EventType == EInputEvent::IE_Released)
		{
			if (bDragging && MyGameInstance && SelectedActor)
			{
				FSaveItem* MySaveItem = MyGameInstance->SaveItems.Find(SelectedActor);
				if (MySaveItem)
				{
					MySaveItem->ItemTransform = SelectedActor->GetActorTransform();
				}
			}

			bDragging = false;
			TActorIterator<APlayerController> Itr(GetWorld());
			Itr->GetPawn()->EnableInput(*Itr);
		}
		Viewport->InvalidateHitProxy();
		//Viewport->Draw();
...
{
	if (bEditMode && bDragging &&SelectedActor)
	{
		const float DragX = (Key == EKeys::MouseX) ? Delta : 0.f;
		const float DragY = (Key == EKeys::MouseY) ? Delta : 0.f;
		FVector DragDelta = FVector(DragX, DragY, 0);

		switch (WidgetMode)
		{
		case WM_Translate:
		{
			FVector Wk = FVector::ZeroVector;

			FVector2D MousePosition(InViewport->GetMouseX(), InViewport->GetMouseY());
			...
			FLocalPlayerIterator Iterator(GEngine, GetWorld());
			FVector		ViewLocation;
			FRotator	ViewRotation;
			FSceneView* View = (*Iterator)->CalcSceneView(&ViewFamily, ViewLocation, ViewRotation, InViewport);
			MyWidget->AbsoluteTranslationConvertMouseMovementToAxisMovement(View, MyWidget->WidgetLocation, MousePosition, Wk);

			FTransform NewTransform = SelectedActor->GetActorTransform();
			NewTransform.SetTranslation(NewTransform.GetTranslation() + Wk);
			SelectedActor->SetActorTransform(NewTransform);
			MyWidget->WidgetLocation = NewTransform.GetTranslation();

			OnLocationChanged.Broadcast();
		}
		break;
		...
		case WM_Rotate:
		{
			FRotator Rot;
			FVector Scale;
			MyWidget->ConvertMouseMovementToAxisMovement(DragDelta, Rot, Scale);
			MyWidget->UpdateDeltaRotation();

			const FRotator OriginalRotation = SelectedActor->GetActorRotation();
			FRotator NewRotation = (Rot.Quaternion() * OriginalRotation.Quaternion()).Rotator();
			SelectedActor->SetActorRotation(NewRotation);

			OnRotationChanged.Broadcast();
		}
		break;
		...
		case WM_Scale:
		{
			...
			FVector NewScale = SelectedActor->GetActorScale() + Scale;
...
void UMyGameViewportClient::Draw(FViewport* InViewport, FCanvas* SceneCanvas)
{
	...
	{
		DebugCanvasSize = FIntPoint(1024, 1024);//GEngine->XRSystem->GetHMDDevice()->GetIdealDebugCanvasRenderTargetSize();
	}
	...
	FMyGameViewDrawer GameViewDrawer(MyWidget);
	//GameViewDrawer.DrawHUD(SceneCanvas);
	...
	FSceneViewFamilyContext ViewFamily(FSceneViewFamily::ConstructionValues(
		...
		EngineShowFlags)
		.SetRealtimeUpdate(true));
	ViewFamily.EngineShowFlags.SetCompositeEditorPrimitives(true);
...
void UMyGameViewportClient::EnterEditMode()
{
	bEditMode = true;
	MyWidget->bDrawWidget = true;
}

void UMyGameViewportClient::ExitEditMode()
{
	bEditMode = false;
	//WidgetMode = WM_None;
	MyWidget->bDrawWidget = false;
	TActorIterator<APlayerController> Itr(GetWorld());
	Itr->GetPawn()->EnableInput(*Itr);
}

void UMyGameViewportClient::SetWidgetMode(int32 Mode)
{
	switch (Mode)
	{
	case 1:
...
void UMyGameViewportClient::ToggleWidgetMode()
{
	switch (WidgetMode)
	{
	case WM_Translate:
...
void UMyGameViewportClient::SetSelectedActor(AActor* InSelectedActor)
{
	if (InSelectedActor)
	{
		SelectedActor = InSelectedActor;
		MyWidget->WidgetLocation = SelectedActor->GetActorLocation();
		if (WidgetMode == WM_None)
		{
			WidgetMode = WM_Translate;
		}
		OnActorSelected.Broadcast(InSelectedActor);
	}
}

void UMyGameViewportClient::ToggleCoordSystemSpace()
{
	if (MyWidget->CustomCoordSystemSpace == COORD_World)
	{
		MyWidget->CustomCoordSystemSpace = COORD_Local;
	}
	else if (MyWidget->CustomCoordSystemSpace == COORD_Local)
	{
		MyWidget->CustomCoordSystemSpace = COORD_World;
	}
}